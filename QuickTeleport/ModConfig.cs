...
{
    /// <summary>
    /// Typed access to <c>muindor.QuickTeleport.cfg</c>.
    ///
    /// Every setting that changes how long a teleport takes is bound through <see cref="Sync"/>:
    /// a server that also runs QuickTeleport with <c>ConfigPriority</c> on decides them, so every
    /// player on it travels at the same speed. Only <see cref="IsDebug"/> stays local.
    /// </summary>
    public sealed class ModConfig
    {
        ...
        public ModConfig(ConfigFile config)
        {
            ...
            Enabled = Sync.Bind("General", "Enabled", true, "Enable this mod. Off = vanilla teleports.");
            IsDebug = config.Bind("General", "IsDebug", false,
                "Log the timeline of every teleport (screen black, area loaded, settled, finished).");

            Mode = Sync.Bind("Teleport", "Mode", TeleportMode.Auto,
            ...
            SpeedMultiplier = Sync.Bind("Teleport", "SpeedMultiplier", 4f,
                new ConfigDescription("Multiplier mode only. 1 = vanilla timing, 4 = four times faster.",
                    new AcceptableValueRange<float>(1f, 100f)));
            FadeDuration = Sync.Bind("Teleport", "FadeDuration", TeleportSettings.DefaultFadeDuration,
            ...
            WaitForAreaLoad = Sync.Bind("Loading", "WaitForAreaLoad", true,
            ...
            WaitForObjects = Sync.Bind("Loading", "WaitForObjects", true,
            ...
            SettleTime = Sync.Bind("Loading", "SettleTime", 0.5f,
                new ConfigDescription(
        ...
        public SyncedEntry<TeleportMode> Mode { get; }

        public SyncedEntry<float> SpeedMultiplier { get; }

        public SyncedEntry<float> FadeDuration { get; }

        public SyncedEntry<bool> WaitForAreaLoad { get; }

        public SyncedEntry<bool> WaitForObjects { get; }

        public SyncedEntry<float> SettleTime { get; }
        ...
        public string Describe()
        {
            ...
            return $"{(Enabled.Value ? "enabled" : "disabled")}, {timing}, fade {FadeDuration.Value.ToString("0.##", culture)} s, {loading}, {Sync.Describe()}";
        }
    }
}
