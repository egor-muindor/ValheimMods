...
{
    /// <summary>
    /// Draws the screen markers with IMGUI: an arrow over the vein (or at the screen edge
    /// pointing at it) and a label with the ore name and distance. IMGUI needs no canvas and
    /// no font asset, so it works in any build. Call from <c>OnGUI</c> only.
    /// </summary>
    internal static class MarkerOverlay
    {
        ...
        public static void Draw(IReadOnlyList<VeinHighlight> highlights, Camera camera, Vector3 playerPosition)
        {
            ...
            foreach (VeinHighlight highlight in highlights)
            {
                ...
                var content = new GUIContent($"{highlight.Kind.DisplayName}  {distance:0} m");
